...
When a `PRODUCT.md` exists, it is the primary source of **user stories and acceptance criteria**. For multi-story features, `verify` **defaults to fanning out parallel computer-use (or isolated browser) subagents per key story** via orchestration, then aggregates per-story pass/fail evidence—rather than one serial mega-walkthrough. Implement and triage workflows pin `warpdotdev/oz-agent-action@v1.0.25` with `cloud: true` and `computer_use: true` so Oz cloud runs can drive a GUI. Verification children also enable computer use; workers may still pick browser automation when that is better for pure web flows. By default each path captures **video** plus keyframe screenshots. Evidence and Oz run links are folded back into triage comments, PR descriptions, or `review.json`.
